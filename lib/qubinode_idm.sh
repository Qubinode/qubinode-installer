...
#DNS_SERVER_NAME=$(awk -F'-' '/idm_server_fqdn_name:/ {print $2; exit}' "${idm_vars_file}" | tr -d '"')
prefix=$(awk '/instance_prefix:/ {print $2;exit}' "${vars_file}")
idm_server_name=$(awk '/idm_server_fqdn_name:/ {print $2;exit}' "${vars_file}")
suffix=$(awk '/idm_server_name_suffix:/ {print $2;exit}' "${idm_vars_file}" |tr -d '"')
idm_server_hostname="$prefix-$suffix"
idm_generated_fqdn="$prefix-$suffix.$domain"
...
function ask_user_for_custom_idm_server () {
    ...
    then
        ...
        then
            ...
            sed -i "s/ask_for_idm_info:.*/ask_for_idm_info: skip/g" "${idm_vars_file}"
            ...
            # Set heading for displayed message
            idm_banner_title="IdM VM deployed and IdM server is installed"
            ...
            # Set IDM FQDN name
            sed -i "s/idm_server_fqdn_name:.*/idm_server_fqdn_name: "$idm_generated_fqdn"/g" "${idm_vars_file}"
...
function qubinode_idm_ask_ip_address () {
    ...
    idm_server_name=$(awk '/idm_server_fqdn_name:/ {print $2;exit}' "${idm_vars_file}" |tr -d '"')
...
function isIdMrunning () {
    ...
    then
        idm_running=false
    elif curl -k -s "https://${idm_server_name}/ipa/config/ca.crt" > /dev/null
    then
        idm_running=true
    else
        idm_running=false
...
function idm_status_message () {
    ...
    printf "   ${yel}****************************************************${end}\n"
    printf "    Webconsole: ${cyn}https://${idm_server_name}/ipa/ui/${end} \n"
...
function qubinode_idm_status () {
    ...
    then
        idm_status_message "${idm_banner_title}"
    else
        #if [ "${deploy_idm_server:-none}" == "no" ]
        #then
        #    idm_status_message "${idm_banner_title}"
        #else
        printf "%s\n" " ${red}Could not verify IdM server availability.${end}"
        printf "%s\n" " ${red}Ensure ${idm_server_name} is available${end}"
        exit 1
        #fi
     fi
}
